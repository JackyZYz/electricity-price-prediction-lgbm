...
3. 对 `00:00` 列缺失采用 `24:00` 回填（或删除）
4. 返回标准 `TimeSeriesRecord` 序列或 `DataFrame[timestamp, value, field_name]`
...
class DatasetCSVReader(BaseDataReader):
    ...
    def __init__(self, dataset_root: str, data_sources: dict):
        ...
        self.data_sources = data_sources  # {table_name: relative_path}

    def read_table(self, table_name: str, start_date: str = None, end_date: str = None) -> pd.DataFrame:
        """
        读取单张宽表并转换为长表。
...
class Preprocessor:
    ...
    def handle_missing(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        缺失值处理策略（按优先级与数据类型）：
        1. 00:00 列：用同天 24:00 回填（约定）
        2. 风光夜间空值：按物理意义填充 0
        3. 短时缺失：前向填充（ffill）
        4. 中间段缺失：线性插值（linear）
        5. 长时间缺失：均值填充（fallback）
        6. 缺失率过高字段（如出清电力 > 40%）：标记为不可用
        """
        pass
...
class TrainPipeline:
    ...
    def _time_series_split(self, data, test_days: int = 30):
        """时序分割：按时间划分，不做shuffle；15min粒度下30天=2880条"""
        pass
```
...
class DailyRunner:
    ...
    def run(self, target_date: str) -> PredictionOutput:
        """
        每日运行流程：
        1. 读取 Dataset/ 最新事前数据（目标日期的预测/计划类数据）
        2. 读取历史事后数据（用于滞后特征，仅 T-1 及之前）
...
flowchart TB
    ...
    subgraph s3["<b>Step 3: 数据预处理</b>"]
        s3d["缺失值填补（00:00用24:00回填；风光夜间填0；ffill → linear → mean fallback）<br/>异常值记录（电价尖峰保留；非电价极端值clip）<br/>多表时间戳对齐（以电价目标为基准，缺失日填充）<br/>fit StandardScaler（保存到 models/scalers/）"]
    end
